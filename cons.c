...
int main(int argc, char **argv)
{
	...
	shm_fd = shm_open(name, O_RDONLY, 0666); //初始化共享内存 只读不负责创建
	...
	for (i = 0; i < 3; i++){
		...
		resc = pthread_create(&con[i],&attr[i], (void *)consume, buffer);
		if(resc != 0){
